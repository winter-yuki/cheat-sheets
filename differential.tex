...
\begin{itemize}
	...
	\begin{tabular}{l}
		...
		\begin{tabular}{|l|}
			...
			\textbf{Решение:} $\displaystyle  \frac{-dy}{M(x)N(y)} = \frac{dx}{P(x)Q(y)}$ \\
			\hline
		...
		\begin{tabular}{|l|}
			...
			4) умножаем $u(x)$ на $v(x)$ \\
			5) \textbf{NB}: при $\forall$ делении возвожна потеря решения! \\
			6) иногда возможно поменять $y$ и $x$: $x = u(y) \cdot v(y)$ \\\\
			
			II. \textit{Бернулли}: $y' + a(x)y = b(x)y^n\qquad(n \neq 1)$ \\
			\textbf{Решение:} \\
			1) делим ур-ние на $y^n$ \qquad 2) замена $\frac{1}{y^{n-1}}=z$ \\
			3) свели к задаче выше \\\\
			
			III. \textit{Риккати}: $y' + a(x)y + b(x)y^2=c(x)$ \\
			\textbf{Решение:} \\
			1) замена: $y = \tilde{y}(x) + z$, где $\tilde{y}(x)$ - частное решение \\
			2)для $y' + 2y^2=\frac{6}{x^2}$ ищем что-то	в виде $y = \frac{a}{x}$ \\
			3)для $y' = y^2 = x^2 - 2x$ ищем что-то в виде $y=ax + b$ \\	
			\hline	
		\end{tabular}
		\\
		//TO DO : 1)в полных диф. 2)инт. множ. 3) Дарбу
	\end{tabular}
